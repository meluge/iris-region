...
(** * Type-and-effect system for reg_lang — Tofte–Talpin rule-27 variant.

This is the **TT rule-27** presentation of RegLang typing on the annotation model
[lang.v], kept for comparison with the primary [typing.v] (which uses the Killer
Document's [∀ρ] masking instead).

It differs from [typing.v] in exactly the way TT's rule 27 differs from the
document's RegTMask: region variables are kept *abstract* (effects and [TRef] are
indexed by region *variables* — [gset string]), and masking is done by an explicit
region context [Δ : gset string] plus set-difference [σ ∖ {x}] with the non-escape
side conditions [x ∉ frv Γ], [x ∉ frv A]. (TT itself has no [Δ]; it uses only the
[frv(TE, μ)] side condition. We make the scope explicit as [Δ] because this is a
*checking* judgement on an already-annotated term, not a translation.) *)
...
(** [typed Δ Γ e A σ]: with region variables [Δ] in scope and term context [Γ],
[e] has type [A] and effect [σ]. *)
Inductive typed : eff → tctx → expr → ty → eff → Prop :=
  ...
  | T_Region Δ Γ x e A σ σ' :                         (* RegTMask  (TT rule 27) *)
    typed ({[ x ]} ∪ Δ) Γ e A σ →
    x ∉ Δ →
    x ∉ frv_ctx Γ →
    x ∉ frv A →
